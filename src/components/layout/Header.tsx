...
interface HeaderProps {
  backHref?: string;
  actions?: ReactNode;
  brandVisibility?: 'always' | 'mobile' | 'desktop' | 'none';
}

export default function Header({ backHref, actions, brandVisibility = 'always' }: HeaderProps) {
  const brandClassName =
    brandVisibility === 'none'
      ? 'hidden'
      : brandVisibility === 'mobile'
        ? 'flex md:hidden'
        : brandVisibility === 'desktop'
          ? 'hidden md:flex'
          : 'flex';
  ...
  return (
    <header className="h-16 border-b border-[var(--border)] bg-[var(--bg-primary)]/80 backdrop-blur-sm sticky top-0 z-40">
      <div className="h-full px-4 md:px-8 flex items-center justify-between relative">
        ...
        <div className={`${brandClassName} items-center gap-4 absolute left-1/2 -translate-x-1/2 md:relative md:left-0 md:translate-x-0`}>
          <Image
